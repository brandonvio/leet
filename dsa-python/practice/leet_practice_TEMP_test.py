...
def test_ipoOffering():
    ...
    A company registers for an IPO. All shares are available on the website for
    bidding during a time frame called the bidding window. At the end of the bidding
    window an auction logic is used to decide how many available shares go to which
    bidder until all shares have been allocated, or all the bidders have received
    the shares they bid for, whichever comes first.

    The bids arrive from users in the form of [userid, # shares,
    bidding price, timestamp] until the bidding window is closed.

    The auction logic assigns shares as follows:
        The bidder with the highest price gets the # of shares they bid for
        If multiple bidders have bid at the same price, the bidders are assigned
        shares in the order in which they places their bids (earliest bids first)
        List the userids of all the users who didn't get even 1 share after all
    ...
    I feel like the solution I came up with is relatively simple.
    It seems to pass all edge cases I can think of.

    Question
    I have found a questionable situation:
    Bid price and times are the same and there aren't enough shares for
    all bidders ie: bids is [[0,2,10,0], [1,2,10,0]] and totalShares is 2.
    It's unclear if 1 share should be given to each, or if userid 0 just gets both.
    """
    class Solution:
