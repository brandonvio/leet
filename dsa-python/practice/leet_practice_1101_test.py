...
def test_reorderLogFiles():
    ...
    class Solution:
        def reorderLogFiles(self, logs: List[str]) -> List[str]:
            # split the log files into two list, letter and digit.
            #   by splitting the string and using the first character of the
            #   second array element. If element is letter goes into letter list
            #   otherwise goes into digit list
            # order the letter group using a lambda and a tuple
            # append the digits list to end of the letter list and return.
            pass
...
def test_maximumUnits():
    ...
    class Solution:
        def maximumUnits(self, boxTypes: List[List[int]], truckSize: int) -> int:
            ...
            # sort the list of boxes by the number of units per box descending.
            # loop through list of boxes adding all the boxes until the truck is full
            # increment the unit counter with total units added
            # once the truck is full return total number of boxes
            NUM_OF_BOXES = 0
            NUM_OF_UNITS_PER_BOX = 1

            boxTypes.sort(key=lambda x: -x[NUM_OF_UNITS_PER_BOX])
            total_units = 0

            for boxes in boxTypes:
                if truckSize > boxes[NUM_OF_BOXES]:
                    total_units += boxes[NUM_OF_BOXES] * boxes[NUM_OF_UNITS_PER_BOX]
                    truckSize -= boxes[NUM_OF_BOXES]
                else:
                    # truck is full
                    total_units += truckSize * boxes[NUM_OF_UNITS_PER_BOX]
                    break
...
def test_two_sum():
    ...
    def twoSum(nums: List[int], target: int) -> List[int]:
        # create a tracker dict to store numbers that have been seen so far
        # store the index with the tracker
        # loop through the variable, perform a diff against target
        #   determine if the diff has been seen
        # if so return the current index and the index of the seen variable
        tracker = {}

        for index, value in enumerate(nums):
            diff_value = target - value
            if diff_value in tracker:
                return [tracker[diff_value], index]
            else:
                tracker[value] = index
