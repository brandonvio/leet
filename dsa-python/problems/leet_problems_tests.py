    ...
    class Solution:
        def addStrings(self, num1: str, num2: str) -> str:
            ...
            while index_1 >= 0 or index_2 >= 0:
                n1 = int(num1[index_1]) if index_1 >= 0 else 0
                n2 = int(num2[index_2]) if index_2 >= 0 else 0

                value = (n1 + n2 + carry) % 10
                # output_arr.append(value)
                output_str = str(value) + output_str
...
def test_validWordAbbreviation():
    ...
    class Solution:
        def validWordAbbreviation(self, word: str, abbr: str) -> bool:
            # keep track of wordIndex and abbrIndex separately
            wordIndex = 0
            abbrIndex = 0
            while wordIndex < len(word) and abbrIndex < len(abbr):
                if word[wordIndex] == abbr[abbrIndex]:
                    abbrIndex += 1
                    wordIndex += 1
                elif ord('1') <= ord(abbr[abbrIndex]) <= ord('9'):
                    numLen = abbrIndex
                    # include ord('0') since we know we don't have a leading zero
                    while numLen < len(abbr) and ord('0') <= ord(abbr[numLen]) <= ord('9'):
                        numLen += 1
                    wordIndex += int(abbr[abbrIndex: numLen])
                    abbrIndex += numLen - abbrIndex
                else:
                    return False

            # return False only if one of the indices reached the end earlier than the other
...
def test_isPalindrome():
    ...
    class Solution:
        def isPalindrome(self, s: str) -> bool:
            # create a lambda that returns lowercase string or None
            # loop through each value in string call lambda
            # if None, continue, else add string to list
            # loop through string, pop character off list, compare
            # if are equal its a palandrome
